...
if (-not (Test-Path "C:\Windows\Temp\VBoxGuestAdditions.iso")) {
	...
	$proc = Start-Process -FilePath "$name\VBoxWindowsAdditions.exe" -ArgumentList "/S" -WorkingDirectory "C:\Windows\Temp\" -Wait -PassThru
	if ($proc.ExitCode -ne 0) {
		Write-Host "Something wrong with installation" -ForegroundColor Red
	}
